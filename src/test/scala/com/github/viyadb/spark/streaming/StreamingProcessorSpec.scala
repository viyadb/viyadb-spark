...
class StreamingProcessorSpec extends UnitSpec with BeforeAndAfter {
  ...
  "StreamingProcessor" should "process all metric types" in {
    ...
    val streamingProcessor = new StreamingProcessor(config)

    val records = recordFactory.createDataFrame(
      ss.sparkContext.makeRDD[Row](
        tsvContent.map(tsv => recordFactory.createRecord("", tsv).get)))
